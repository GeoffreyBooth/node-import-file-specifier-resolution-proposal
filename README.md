...
- **“Dual mode” packages**
  ...
  The ESM and CommonJS module systems have independent namespaces.  Therefore the ESM and CommonJS versions of a module have separate identities when instantiated and both may be loaded into the same application at the same time.  Without this proposal, the module loader's invariant was: for a single resolved location, the corresponding source text will be loaded at-most once within a Node application.  With this proposal, the invariant becomes: for a single resolved location, each version (ESM or CommonJS) of the corresponding source text will be loaded at-most once within a Node application.
